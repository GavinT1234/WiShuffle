...
import { extractVideoId, getVideoInfo } from '../../services/youtubeService.js';
...
async function getPlaylistWithTitles(videoIds) {
  const playlistWithTitles = [];

  for (const videoId of videoIds) {
    // Try to get from cache first
    const cachedInfo = await redis.get(`video:${videoId}:info`);

    if (cachedInfo) {
      const info = JSON.parse(cachedInfo);
      playlistWithTitles.push(info);
    } else {
      // If not cached, fetch it
      const videoInfo = await getVideoInfo(videoId);
      await redis.set(`video:${videoId}:info`, JSON.stringify(videoInfo), { EX: 86400 });
      playlistWithTitles.push(videoInfo);
    }
  }

  return playlistWithTitles;
}
...
export function registerRoomHandlers(io, socket) {
  ...
  try {
    ...
    // ✅ Get current playback state and playlist
    try {
      const playbackData = await redis.hGetAll(`room:${roomId}:playback`);
      const playlistVideoIds = await redis.lRange(`room:${roomId}:playlist`, 0, -1);

      console.log(`📋 Playlist for room ${roomId}:`, playlistVideoIds);

      // ✅ Send current video if exists
      if (playbackData && playbackData.videoId) {
        console.log(`📤 Sending current playback to new user ${socket.user.id}:`, playbackData);
        ...
        console.log(`📤 Emitting room:video_load to new joiner:`, payload);
        socket.emit('room:video_load', payload);
      }

      // ✅ Send playlist with titles
      const playlistWithTitles = await getPlaylistWithTitles(playlistVideoIds);
      console.log(`📤 Sending playlist to new user:`, playlistWithTitles);
      socket.emit('room:queue_updated', { queue: playlistWithTitles, roomId });
  ...
  try {
    ...
    // ✅ FIRST: Determine the title (fetch if needed)
    let finalTitle;
    if (title) {
      // Title provided (from search)
      finalTitle = title;
      console.log(`   Using provided title: ${finalTitle}`);
    } else {
      // No title, fetch from YouTube
      console.log(`   No title provided, fetching...`);
      const videoInfo = await getVideoInfo(videoId);
      finalTitle = videoInfo.title;
      console.log(`   Fetched title: ${finalTitle}`);
    }

    // ✅ THEN: Create song object with the correct title
    const song = {
      videoId,
      title: finalTitle,
    };
    ...
    // Cache video info
    await redis.set(`video:${videoId}:info`, JSON.stringify(song), { EX: 86400 });
    ...
    // Broadcast updated playlist with titles
    const updatedPlaylist = await redis.lRange(`room:${roomId}:playlist`, 0, -1);
    const playlistWithTitles = await getPlaylistWithTitles(updatedPlaylist);
    io.to(`room:${roomId}`).emit('room:queue_updated', { queue: playlistWithTitles, roomId });

    if (ack) ack({ ok: true });
    console.log(`✅ Video queued: ${song.title}`);
    ...
    try {
      ...
      if (nextPlaylist.length === 0) {
        ...
        if (cachedInfo) {
          ...
          const videoInfo = await getVideoInfo(nextVideoId);
          videoTitle = videoInfo.title;
          await redis.set(`video:${nextVideoId}:info`, JSON.stringify(videoInfo), { EX: 86400 });
        }
      ...
      const playlistWithTitles = await getPlaylistWithTitles(nextPlaylist);
      io.to(`room:${roomId}`).emit('room:queue_updated', { queue: playlistWithTitles, roomId });
